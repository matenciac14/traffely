...
import { MoreHorizontalIcon, PowerIcon, CheckCircleIcon, TrashIcon, Loader2Icon } from "lucide-react"
...
export default function WorkspaceActions({ workspaceId, isActive, billingStatus }: Props) {
  const router = useRouter()
  const [open, setOpen] = useState(false)
  const [loading, setLoading] = useState(false)

  async function action(type: "toggle" | "billing" | "delete", value?: string) {
    setLoading(true)
    setOpen(false)
  ...
  return (
    <div className="relative">
      ...
      >
        {loading ? <Loader2Icon className="w-4 h-4 animate-spin" /> : <MoreHorizontalIcon className="w-4 h-4" />}
        Acciones
      </button>
