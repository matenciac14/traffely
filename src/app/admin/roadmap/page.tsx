const ROADMAP: { fase: string; status: "done" | "in-progress" | "pending"; items: { label: string; done: boolean }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Campos del brief estratégico en wizard Step 2 (objetivo, público, insight, tono, CTA…)", done: false },
      { label: "Guardar campo 'empresa' en autosave a DB", done: false },
      { label: "Indicador de completitud del brief (X/8 campos) antes de generar", done: false },
      { label: "StepPromptOutput: CTA actualizado — generar brief IA inline, no en Claude.ai", done: false },
      { label: "Step 7 Equipo: clarificar que es para el prompt, no invita usuarios reales", done: false },
      { label: "Piezas auto-creadas en Step 5 heredan modelo seleccionado en Step 4", done: false },
    ],
  },
]
